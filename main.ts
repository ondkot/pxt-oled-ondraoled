...
namespace OLED {
    ...
    let buffer: Array<Array<number>>; // buffer[col][row]
    let toupdate: Array<Array<number>>; // [[row, page], [row, page]]
    ...
    export function clear() {
        ...
        charX = xOffset
        charY = yOffset
        buffer = new Array<Array<number>>(width).fill(new Array<number>(height).fill(0))
        toupdate = new Array<Array<number>>();
    }
    ...
    export function update() {
        toupdate.forEach(updateelement => {
            command(SSD1306_SETCOLUMNADRESS);
            command(updateelement[0]);
            command(updateelement[0] + 1);
            command(SSD1306_SETPAGEADRESS);
            command(updateelement[1]);
            command(updateelement[1] + 1);
            let dataBuf = pins.createBuffer(2);
            dataBuf[0] = 0x40;
            dataBuf[1] = 0;
            for (let y = 0; y < 7; y++) {
                dataBuf[1] |= (updateelement[1] * 8 + y) << y
            }
            pins.i2cWriteBuffer(chipAdress, dataBuf, false)
        })
    }
    function drawShape(pixels: Array<Array<number>>) {
        pixels.forEach(pixel => {
            buffer[pixel[0]][pixel[1]] = 1;
            if (toupdate.indexOf([pixel[0], Math.floor(pixel[1] / 8)]) != 1) {
                toupdate.push([pixel[0], Math.floor(pixel[1] / 8)]);
            }
        });
    }
    ...
    //% block="show pixel at x $x y $y"
    //% x.defl=0
    //% y.defl=0
    //% weight=1
    export function showPixel(x: number, y: number) {
        let pixels: Array<Array<number>> = []
        pixels.push([x, y]);
        drawShape(pixels);
    }
